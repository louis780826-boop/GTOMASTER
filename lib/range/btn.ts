...
function btnRule(p: RuleParams) {
  ...
  if (suited) {
    if (hiRank === "A") {
      if (["K","Q","J","T","9","8","7","6","5"].includes(loRank)) {
        return { action: "raise" };
      }
    ...
    if (isConnector && highIndex <= 9) {
      return { action: "raise" };
    }
    if (oneGap && highIndex <= 9) {
      return { action: "mix", freq: 0.7 };
    }
    if (twoGap && highIndex <= 8) {
...
export const btnRange: RangeMatrix = buildRange(btnRule);
